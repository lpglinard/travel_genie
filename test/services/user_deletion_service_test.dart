...
void main() {
  group('UserDeletionService', () {
    ...
    test('should provide Firebase-based user deletion', () {
      ...
      expect(service.deleteAllUserData('test-user-id'), isA<Future<UserDeletionResponse>>());
    });

    test('should handle unauthenticated user scenario', () async {
      // Test that the service properly handles when no user is authenticated
      
      // Arrange - No authenticated user (Firebase Auth will return null)
      const testUserId = 'test-user-123';
      
      // Act - Attempt to delete user data when no user is authenticated
      final result = await service.deleteAllUserData(testUserId);
      
      // Assert - Should return failure response for unauthenticated user
      expect(result, isA<UserDeletionResponse>());
      expect(result.success, isFalse);
      expect(result.userId, testUserId);
      expect(result.errorMessage, contains('User not authenticated'));
      expect(result.message, contains('No authenticated user found'));
